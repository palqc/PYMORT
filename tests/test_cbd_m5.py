...
def test_logit_stability_and_bounds():
...
def test_build_cbd_design():
    ...
    with pytest.raises(ValueError):
        _build_cbd_design(np.array([[70, 71]]))


def test_fit_cbd_and_reconstruct():
    ages, q = _toy_q()
...
def test_fit_cbd_invalid_inputs():
    ages, q = _toy_q()
    with pytest.raises(ValueError):
        fit_cbd(q[:, :2], ages)  # shape mismatch
    with pytest.raises(ValueError):
        fit_cbd(np.array([0.1, 0.2]), ages)  # not 2D
    bad_q = q.copy()
    bad_q[0, 0] = -0.1
    with pytest.raises(ValueError):
        fit_cbd(bad_q, ages)
...
def test_estimate_rw_params_cbd_and_helper():
    kappa = np.array([0.0, 0.1, 0.2, 0.25])
    mu, sigma = _estimate_rw_params(kappa)
    assert np.isfinite(mu) and np.isfinite(sigma)
    with pytest.raises(ValueError):
        _estimate_rw_params(np.array([0.1]))
...
def test_cbdm5_class_fit_predict_simulate():
    ages, q = _toy_q()
...
def test_cbdm5_simulate_errors():
    model = CBDM5()
    with pytest.raises(ValueError):
        model.predict_q()
    with pytest.raises(ValueError):
